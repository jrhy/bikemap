...
2. Allow `https://jrhy.github.io` in the token's Mapbox URL restrictions, without `/bikemap/`. Browsers normally send only the origin as the referrer on cross-origin requests; restricting the token to a repository path can reject real tile requests with `403`. For another host, allow that site's origin. The token needs public style and tile read access. See [Mapbox token documentation](https://docs.mapbox.com/accounts/guides/tokens/).
